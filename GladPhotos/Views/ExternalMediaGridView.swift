...
private struct ExternalMediaDaySection: Identifiable, Sendable {
    let date: Date
    let items: [ExternalMediaItem]

    var id: Date { date }

    nonisolated static func make(from items: [ExternalMediaItem]) -> [ExternalMediaDaySection] {
...
private struct ExternalMediaGridSnapshot: Sendable {
    let items: [ExternalMediaItem]
    let itemsByMonth: [MediaMonth: [ExternalMediaItem]]
    let daySections: [ExternalMediaDaySection]
    let undatedItems: [ExternalMediaItem]

    nonisolated static let empty = ExternalMediaGridSnapshot(
        items: [],
        itemsByMonth: [:],
        daySections: [],
        undatedItems: []
    ...
    nonisolated static func make(
        from newItems: [ExternalMediaItem],
        monthDate: Date?,
        photographyFilter: PhotographyFilter,
        photographyRecords: [String: PhotographyAnalysisRecord]
    ) -> ExternalMediaGridSnapshot {
        ...
        var monthCache: [MediaMonth: [ExternalMediaItem]] = [:]
        for item in sortedItems {
            guard let date = item.displayDate else { continue }
            monthCache[MediaMonth(date), default: []].append(item)
        }

        let unfilteredItems = monthDate.map { monthCache[MediaMonth($0)] ?? [] } ?? sortedItems
        let visibleItems: [ExternalMediaItem]
        if photographyFilter == .all {
            visibleItems = unfilteredItems
        } else {
            visibleItems = unfilteredItems.filter { item in
                guard item.mediaType != .video else { return false }
                let record = photographyRecords[item.url.standardizedFileURL.path]
                return photographyFilter.includes(record?.effectiveTag ?? .unknown)
            }
        }

        return ExternalMediaGridSnapshot(
            items: sortedItems,
            itemsByMonth: monthCache,
...
nonisolated private struct ExternalGridItemFrame: Equatable, Sendable {
    let id: ExternalMediaItem.ID
    let index: Int
    let minY: CGFloat
    let maxY: CGFloat

    var centerY: CGFloat { (minY + maxY) / 2 }
    func intersects(viewportHeight: CGFloat) -> Bool {
        maxY >= 0 && minY <= viewportHeight
    }
}

nonisolated private struct ExternalGridPreheatRequest: Equatable, Identifiable, Sendable {
    enum Zone: String, Sendable {
        case forward
        case backward
    }
    ...
    var id: String { "\(zone.rawValue)#\(item.id)" }
}

nonisolated private struct ExternalGridViewportPlan: Equatable, Sendable {
    let visibleIDs: Set<ExternalMediaItem.ID>
    let finalEligibleIDs: Set<ExternalMediaItem.ID>
    let preheatIDs: Set<ExternalMediaItem.ID>
    let preheatRequests: [ExternalGridPreheatRequest]
    let isFastScroll: Bool

    static let empty = ExternalGridViewportPlan(
        visibleIDs: [],
        finalEligibleIDs: [],
        preheatIDs: [],
        preheatRequests: [],
    ...
    var preheatSignature: String {
        preheatRequests.map(\.id).joined(separator: "|")
    }
}
...
struct ExternalMediaGridView: View {
    ...
    @State private var gridSnapshot = ExternalMediaGridSnapshot.empty
    ...
    @State private var finalThumbnailLoadID = UUID()
    @State private var finalThumbnailUpgradeTask: Task<Void, Never>?
    @State private var startupFinalsAllowed = false
    @State private var startupFinalEnableTask: Task<Void, Never>?
    @State private var viewportSize: CGSize = .zero
    @State private var itemFrames: [ExternalMediaItem.ID: ExternalGridItemFrame] = [:]
    @State private var viewportPlan = ExternalGridViewportPlan.empty
    @State private var previousVisibleCenterIndex: Int?
    @State private var preheatTask: Task<Void, Never>?

    private let scrollCoordinateSpace = "external-media-grid-scroll"
    private static let inactiveFinalLoadID = UUID(uuidString: "00000000-0000-0000-0000-000000000000")!
    ...
    private var columns: [GridItem] {
        [
            GridItem(
                .adaptive(minimum: thumbnailWidth, maximum: thumbnailWidth),
                spacing: 2
            )
        ]
    }
    ...
    private var items: [ExternalMediaItem] {
        gridSnapshot.items
    }

    private var itemsByMonth: [MediaMonth: [ExternalMediaItem]] {
        gridSnapshot.itemsByMonth
    }

    private var daySections: [ExternalMediaDaySection] {
        gridSnapshot.daySections
    }

    private var undatedItems: [ExternalMediaItem] {
        gridSnapshot.undatedItems
    }
    ...
    private var bottomItemID: ExternalMediaItem.ID? {
    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
                ...
                if !folder.accessState.isAvailable {
                    ...
                    ScrollViewReader { proxy in
                        ...
                        .onScrollPhaseChange { _, newPhase in
                            updateScrollState(isScrolling: newPhase.isScrolling)
            ...
            .onDisappear {
                ...
                folderWatcher.stop()
                finalThumbnailUpgradeTask?.cancel()
                startupFinalEnableTask?.cancel()
                preheatTask?.cancel()
        ...
        LazyVStack(alignment: .leading, spacing: 16) {
            ForEach(daySections) { section in
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    mediaGrid(section.items, indexOffset: displayedStartIndex(for: section.id))
                }
            }
            ...
            if !undatedItems.isEmpty {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    mediaGrid(undatedItems, indexOffset: daySections.reduce(0) { $0 + $1.items.count })
                }
            }
        }
        .padding(2)
    ...
    private func mediaGrid(_ mediaItems: [ExternalMediaItem], indexOffset: Int) -> some View {
        LazyVGrid(columns: columns, spacing: 2) {
            ForEach(Array(mediaItems.enumerated()), id: \.element.stableMediaID) { offset, item in
                mediaGridCell(item, itemIndex: indexOffset + offset)
            }
        }
    }

    private func mediaGridCell(_ item: ExternalMediaItem, itemIndex: Int) -> some View {
        let allowsFinal = startupFinalsAllowed &&
            !isScrolling &&
            viewportPlan.finalEligibleIDs.contains(item.id)
        return ExternalMediaGridCell(
            item: item,
            thumbnailService: thumbnailService,
            ...
            allowsFinalThumbnail: allowsFinal,
            finalLoadID: allowsFinal ? finalThumbnailLoadID : Self.inactiveFinalLoadID,
            open: {
        ...
        .equatable()
        .onGeometryChange(for: ExternalGridItemFrame.self) { proxy in
            let frame = proxy.frame(in: .named(scrollCoordinateSpace))
            return ExternalGridItemFrame(
                id: item.id,
                index: itemIndex,
                minY: frame.minY,
                maxY: frame.maxY
        ...
        .onDisappear {
            removeItemFrame(for: item.id)
        }
    }

    private var orderedDisplayedItems: [ExternalMediaItem] {
        daySections.flatMap(\.items) + undatedItems
    }

    private func displayedStartIndex(for sectionID: ExternalMediaDaySection.ID) -> Int {
        guard let sectionIndex = daySections.firstIndex(where: { $0.id == sectionID }),
              sectionIndex > 0 else { return 0 }
        return daySections.prefix(sectionIndex).reduce(0) { $0 + $1.items.count }
    }

    private func updateViewportSize(_ size: CGSize) {
        guard abs(size.width - viewportSize.width) >= 1 ||
            abs(size.height - viewportSize.height) >= 1
        else { return }
        viewportSize = size
        rebuildViewportPlan()
    }

    private func updateItemFrame(_ frame: ExternalGridItemFrame) {
        guard itemFrames[frame.id] != frame else { return }
        itemFrames[frame.id] = frame
        rebuildViewportPlan()
    }

    private func removeItemFrame(for id: ExternalMediaItem.ID) {
        guard itemFrames[id] != nil else { return }
        itemFrames[id] = nil
        rebuildViewportPlan()
    }

    private func rebuildViewportPlan() {
        let viewportHeight = max(1, viewportSize.height)
        let visibleFrames = itemFrames.values
            .filter { $0.intersects(viewportHeight: viewportHeight) }
            .sorted { $0.index < $1.index }
        guard !visibleFrames.isEmpty else {
            publishViewportPlan(.empty)
            previousVisibleCenterIndex = nil
            return
        }

        let viewportCenter = viewportHeight / 2
        let centerFrame = visibleFrames.min {
            abs($0.centerY - viewportCenter) < abs($1.centerY - viewportCenter)
        }
        let centerIndex = centerFrame?.index ?? visibleFrames[visibleFrames.count / 2].index
        let previousCenter = previousVisibleCenterIndex
        let scrollsForward = previousCenter.map { centerIndex >= $0 } ?? true
        let indexDelta = previousCenter.map { abs(centerIndex - $0) } ?? 0
        let visibleIDs = Set(visibleFrames.map(\.id))
        let visibleIndexRange = (visibleFrames.first!.index)...(visibleFrames.last!.index)
        let fastThreshold = max(visibleFrames.count, estimatedItemsPerScreen() / 2, 8)
        let isFastScroll = isScrolling && indexDelta > fastThreshold
        previousVisibleCenterIndex = centerIndex

        let preheatRequests = makePreheatRequests(
            visibleIndexRange: visibleIndexRange,
            scrollsForward: scrollsForward,
            isFastScroll: isFastScroll
        )
        let preheatIDs = Set(preheatRequests.map(\.item.id))
        publishViewportPlan(ExternalGridViewportPlan(
            visibleIDs: visibleIDs,
            finalEligibleIDs: visibleIDs,
            preheatIDs: preheatIDs,
            preheatRequests: preheatRequests,
            isFastScroll: isFastScroll
        ))
    }

    private func estimatedItemsPerScreen() -> Int {
        let columns = max(1, Int((viewportSize.width / max(thumbnailWidth + 2, 1)).rounded(.down)))
        let rows = max(1, Int((viewportSize.height / max(thumbnailWidth + 2, 1)).rounded(.up)))
        return max(1, columns * rows)
    }

    private func makePreheatRequests(
        visibleIndexRange: ClosedRange<Int>,
        scrollsForward: Bool,
        isFastScroll: Bool
    ) -> [ExternalGridPreheatRequest] {
        let allItems = orderedDisplayedItems
        guard !allItems.isEmpty else { return [] }

        let screenCount = estimatedItemsPerScreen()
        let forwardCount = isFastScroll ? screenCount : screenCount * 2
        let backwardCount = isFastScroll ? max(screenCount / 3, 4) : screenCount
        let maxIndex = allItems.count - 1

        let forwardRange = rangeAfter(
            visibleIndexRange.upperBound,
            count: forwardCount,
            maxIndex: maxIndex
        )
        let backwardRange = rangeBefore(
            visibleIndexRange.lowerBound,
            count: backwardCount
        )
        let firstZone: ExternalGridPreheatRequest.Zone = scrollsForward ? .forward : .backward
        let secondZone: ExternalGridPreheatRequest.Zone = scrollsForward ? .backward : .forward

        var requests: [ExternalGridPreheatRequest] = []
        appendPreheatRequests(
            from: firstZone == .forward ? forwardRange : backwardRange,
            zone: firstZone,
            allItems: allItems,
            requests: &requests
        )
        appendPreheatRequests(
            from: secondZone == .forward ? forwardRange : backwardRange,
            zone: secondZone,
            allItems: allItems,
            requests: &requests
        )
    ...
    private func rangeAfter(_ index: Int, count: Int, maxIndex: Int) -> [Int] {
        guard count > 0, index < maxIndex else { return [] }
        let start = index + 1
        let end = min(maxIndex, index + count)
        return Array(start...end)
    }

    private func rangeBefore(_ index: Int, count: Int) -> [Int] {
    ...
    private func appendPreheatRequests(
        from indices: [Int],
        zone: ExternalGridPreheatRequest.Zone,
        allItems: [ExternalMediaItem],
        requests: inout [ExternalGridPreheatRequest]
    ) {
        for index in indices where allItems.indices.contains(index) {
            let item = allItems[index]
            requests.append(ExternalGridPreheatRequest(item: item, zone: zone, order: requests.count))
        }
    }

    private func publishViewportPlan(_ plan: ExternalGridViewportPlan) {
        guard plan != viewportPlan else { return }
        viewportPlan = plan
        ExternalMediaScrollDiagnostics.shared.recordViewport(
            visible: plan.visibleIDs.count,
            preheat: plan.preheatIDs.count
        )
        schedulePreheat(for: plan)
    }

    private func schedulePreheat(for plan: ExternalGridViewportPlan) {
        preheatTask?.cancel()
        guard !plan.preheatRequests.isEmpty else { return }
        let requests = plan.preheatRequests
    ...
    private func restoreCacheAndWatch() async {
        ...
        if !initialItems.isEmpty {
            await restoreItems(initialItems)
            await scan()
        } else if let indexed = await scanner.indexedItems(folderID: folder.id, folderURL: url),
                  !indexed.items.isEmpty {
            await restoreItems(indexed.items)
            await scan()
        } else if let cached = await scanner.cachedItems(for: url) {
            await setItems(cached)
            await scan()
        } else {
            await scan()
        }
    ...
    private func updateScrollState(isScrolling newValue: Bool) {
        isScrolling = newValue
        finalThumbnailUpgradeTask?.cancel()
        guard !newValue else { return }
        finalThumbnailUpgradeTask = Task { @MainActor in
            try? await Task.sleep(for: .milliseconds(150))
            guard !Task.isCancelled else { return }
            finalThumbnailLoadID = UUID()
        }
    }
    ...
    private func deleteItem(_ item: ExternalMediaItem) async throws {
        do {
            ...
            await setItems(items.filter { !sourceURLs.contains($0.url) })
            if let folderURL = folder.url {
    ...
    private func scan() async {
        ...
        do {
            ...
            await setItems(try await scanner.scan(folderID: folder.id, folderURL: url))
            PerformanceLogger.log(
                "external-index-publish",
                duration: publishStart.duration(to: .now),
                details: "items=\(items.count)"
            )
    ...
    private func setItems(_ newItems: [ExternalMediaItem]) async {
        let groupingStart = ContinuousClock.now
        let isFirstPublish = items.isEmpty && !newItems.isEmpty
        let monthDate = self.monthDate
        let photographyFilter = self.photographyFilter
        let photographyRecords = self.photographyRecords
        let snapshot = await Task.detached(priority: .userInitiated) {
            ExternalMediaGridSnapshot.make(
                from: newItems,
                monthDate: monthDate,
                photographyFilter: photographyFilter,
                photographyRecords: photographyRecords
            )
        }.value
        let applyStart = ContinuousClock.now
        apply(snapshot)
        ExternalMediaStartupDiagnostics.shared.recordSnapshotApply(applyStart.duration(to: .now))
        if isFirstPublish {
            beginStartupThumbnailProtection(itemCount: newItems.count)
        }
    ...
    private func restoreItems(_ cachedItems: [ExternalMediaItem]) async {
        ...
        await setItems(cachedItems)
    }

    private func beginStartupThumbnailProtection(itemCount: Int) {
        startupFinalsAllowed = false
        ExternalMediaStartupDiagnostics.shared.start(itemCount: itemCount, folderURL: folder.url)
        startupFinalEnableTask?.cancel()
        startupFinalEnableTask = Task { @MainActor in
            try? await Task.sleep(for: .milliseconds(450))
            guard !Task.isCancelled else { return }
            startupFinalsAllowed = true
            finalThumbnailLoadID = UUID()
        }
    }

    private func apply(_ snapshot: ExternalMediaGridSnapshot) {
    ...
    private func rebuildDisplayedSections() {
        ...
        apply(ExternalMediaGridSnapshot(
            items: items,
        ...
        PerformanceLogger.log(
            "grouping",
            duration: groupingStart.duration(to: .now),
            details: "visible=\(visibleItems.count) sections=\(gridSnapshot.daySections.count)"
        )
    }
    ...
    private func scrollToLatest(proxy: ScrollViewProxy) {
        ...
        ExternalMediaStartupDiagnostics.shared.recordScrollToLatest(scrollStart.duration(to: .now))
        hasPerformedInitialScroll = true
    }
