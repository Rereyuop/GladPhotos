...
struct PhotoGridView: View {
    ...
    private var undatedAssets: [PhotoAssetItem] {
        assets.filter { $0.asset.creationDate == nil }
    }
    ...
    private func scrollToSelectedDate(proxy: ScrollViewProxy) {
        ...
        guard daySections.contains(where: { $0.id == target }) else {
            return
        }
    ...
    private func updateActiveDay(from offsets: [Date: CGFloat]) {
        ...
        let centeredCandidate = activeDayCandidate(
            from: offsets,
            threshold: activeDayReferenceY
        )

        guard let activeDay else {
            if let centeredCandidate {
                self.activeDay = centeredCandidate
            }
            return
        }

        let orderedDays = daySections.map(\.id)
        guard let activeIndex = orderedDays.firstIndex(of: activeDay) else {
            if let centeredCandidate, self.activeDay != centeredCandidate {
                self.activeDay = centeredCandidate
            }
            return
        }

        let downwardCandidate = activeDayCandidate(
            from: offsets,
            threshold: activeDayReferenceY - halfHysteresis
        )
        let upwardCandidate = activeDayCandidate(
            from: offsets,
            threshold: activeDayReferenceY + halfHysteresis
        )

        let newActiveDay: Date?
        if let downwardCandidate,
           let candidateIndex = orderedDays.firstIndex(of: downwardCandidate),
           candidateIndex > activeIndex {
            newActiveDay = downwardCandidate
        } else if let upwardCandidate,
                  let candidateIndex = orderedDays.firstIndex(of: upwardCandidate),
                  candidateIndex < activeIndex {
            newActiveDay = upwardCandidate
        } else {
            newActiveDay = nil
        }

        if let newActiveDay, activeDay != newActiveDay {
            self.activeDay = newActiveDay
        }
    }
    ...
    private func activeDayCandidate(
